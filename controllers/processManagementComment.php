...
function display_comments($company_id)
{
    ...
    while ($row = $result_set->fetch_assoc()) {
        ...
        $tbody_data .= '<tr data-name="test">';
        $tbody_data .= '  <td class="user_name">' . $row["username"] . '</td>';
        ...
        if (!$comment_null) {
            $tbody_data .= '  <td>' . $row['comment'] . '</td>';
            $tbody_data .= '  <td><button class="btn btn-sm contrast btn-edit-comment" name="test" data-user_id="' . $row["user_id"] . '" >Edit</button></td>';
        } else {
            $tbody_data .= '  <td class="text-red">No comment</td>';
            $tbody_data .= '  <td><button class="btn btn-sm contrast btn-add-comment" data-user_id="' . $row["user_id"] . '" onclick="addComments(this)">Add</button></td>';
        }
