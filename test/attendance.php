<?php
	...
	$sql = "SELECT * FROM job_t, scope_t WHERE job_t.scopeid = scope_t.scopeid AND HotelID='$phphotelid' AND job_t.JobDate >= (NOW( ) - INTERVAL 4 DAY) AND job_t.JobDate <= (NOW( ) + INTERVAL 3 DAY) AND job_t.JobStatus<2 order by JobDate, JobStartTime";
	...
	if($count>0){
		while($row = mysql_fetch_array($result))
		{
			...
			$tbody_data.="    <td><a class='btn contrast btn-sm' href='checkin.php?jobid=".$row['JobID']."' />Check In</a> &nbsp;&nbsp;";
        		$tbody_data.="        <a class='btn contrast btn-sm' href='checkout.php?jobid=".$row['JobID']."' />Check Out</a>";
			$tbody_data.='    </td>';
			$tbody_data.='</tr>';
		}
	}
    ...
    <div class='modal-dialog'>
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                ...
                <p>5</p>
                <h4>Number Fulfilled</h4>

                <p>3</p>
                <h4>Start Date/End Date</h4>

                <p>12 Nov 2013 - 13 Nov 2013</p>
                <h4>Time Slots Available (Note: functionality to allow employers to add multiple time slots in one job
                    request is under planning)</h4>

                <p>13:00 to 15:00</p>
                <hr>
                <h4>Location (text below to be replaced by Google Map, if technically feasible)</h4>
